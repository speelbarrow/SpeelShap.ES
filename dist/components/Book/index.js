...
export default class Book extends withShadow(CustomElement) {
    ...
    async #pages() {
        if (!this.#memoPages || this.#updated) {
            this.#memoPages = Array.from(this.children).filter(e => e instanceof Page);
            this.#updated = false;
        }
        return this.#memoPages;
    }
    async #unsetPage(page) {
        if (page)
            page.slot = "";
    }
    async #switch(pages, index, old = this.#page?.[0]) {
        this.#unsetPage(old);
        pages[index].slot = "current";
        this.#page = [pages[index], index];
    }
    async switchByIndex(index, pages) {
    ...
    async attribute_wraparound(_, wraparound) {
